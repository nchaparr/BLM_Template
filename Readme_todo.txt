...
3) Fig 2 deltah_gamma:  I think we should change \delta h to just \delta so we keep the same symbol as garcia
   ...
   since each symbol separately has a definition, so you can't tell a jump from a product.  Still working on this.
   

4) Fig 3 rss_fit_hight:  text leters inside figure. Don't we mean h, not h_0 (Fig. 1) in this discussion

5) Fig. 4 ML_Height_hist_5: text letters inside figure

6) Fig. 5 scaled_downwarm_theta:  text letters inside boxes

7) Fig. 6: theta_grad_profs: text letters inside boxes -- change axis labels for c) and d) per Douw's comment
       c) and d) y axes are different?
